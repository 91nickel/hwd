...
- Module folder `NN-name/` (NN = course number):
  - `README.md` — human-facing (Russian): table of contents + description.
    Lecture table columns: № | Тема (linked to the .md file) | О чём (1-line summary) | Задания (link to `tasks/NN.M/`).
  ...
  - `NN-*.md` — lecture notes / конспекты. Each lecture note ends with:
    ...
    2. "Задания" section — single link `[Все задания →](tasks/NN.M/)`.
  - `tasks/NN.M/` — one folder per lecture:
    - `README.md` — task table: № | Задание (linked to the .md file).
    - `NN.M.K-slug.md` — one file per task, containing both condition and answer.
      Format: `# Задание NN.M.K — Title`, then condition, then `## Ответ` section.
      If a task has an AI dialogue, add `## Чат с ИИ` before `## Ответ`.
    - `code/` — runnable `.py` files, only when the task explicitly requires executable Python code.
...
1. Read CURRENT.md → get {module} folder and current lecture NN.M.
2. Read inbox.txt → get task text and number (if given).
   If no number: check {module}/tasks/NN.M/ for existing tasks, increment the last one.
3. Slug: transliterate the task title to lowercase-kebab-case (Russian→latin).
4. Create {module}/tasks/NN.M/NN.M.K-slug.md:
   # Задание NN.M.K — Title
   {task text verbatim}
   ## Ответ
   (blank — to be filled after user submits solution)
5. Append to {module}/tasks/NN.M/README.md task table:
   | NN.M.K | [Title](NN.M.K-slug.md) |
   If tasks/NN.M/README.md doesn't exist yet, create it with header and table.
6. Append to reference/course-map.md under Tasks:
   - [NN.M.K] Title — one-line essence → {module}/tasks/NN.M/
7. Overwrite inbox.txt with empty string.
8. git add, commit "Add task NN.M.K — Title", push to master.

## Workflow (after every lecture or task)
0. Every lecture note ends with a "Задания" section: single link `[Все задания →](tasks/NN.M/)`.
1. Format the material (конспект / NN.M.K-slug.md).
2. Update {module}/tasks/NN.M/README.md task table.
3. Update the module README.md (Задания column in lecture table).
4. Update `reference/course-map.md`.
